...
from OpenAlumni.Batch import extract_profil_from_imdb, extract_profil_from_unifrance, extract_film_from_imdb, \
	...
	extract_casting_from_imdb, extract_tags_from_imdb, extract_nature_from_imdb, exec_batch, extract_profil_from_filmdoc
...
def test_unifrance_get_profil(profils=PROFILS):
	for p in profils:
		rc_unifrance=extract_profil_from_unifrance(p["name"],refresh_delay=0)
		assert len(rc_unifrance["links"])>=p["unifrance"]["links"]

def test_unifrance_get_film(movies=MOVIES):
	for m in movies.keys():
		log("Recherche de "+m)
		rc_unifrance=extract_film_from_unifrance(m,refresh_delay=1)
		if rc_unifrance:
			assert index_string(rc_unifrance["title"])==index_string(m),"Les titres sont différents "+rc_unifrance["title"]



def test_find_profil_from_imdb(profils=PROFILS):
	for p in profils:
		firstname=p["firstname"] if "firstname" in p else p["name"].split(" ")[0]
		lastname=p["lastname"] if "lastname" in p else p["name"].split(" ")[1]
		_p=extract_profil_from_imdb(lastname,firstname,1,"",imdbBase=MongoBase(IMDB_DATABASE_SERVER))
...
def test_extract_movies_from_profil(query=PROFILS[0], refresh_delay=10):
	...
	if "filmdoc" in query:
		rc_filmdoc=extract_profil_from_filmdoc(query["name"],refresh_delay=refresh_delay)
