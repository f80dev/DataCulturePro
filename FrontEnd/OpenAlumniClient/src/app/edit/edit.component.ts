...
export class EditComponent implements OnInit,OnDestroy {
  ...
  refresh_works() {
    ...
    this.api._get("extraworks", "profil__id=" + this.profil.id, 600).subscribe((r: any) => {
      ...
      for (let w of this.works) {
        w.notification_color=w.error_notification!='' ? 'red' : 'white'
        if (w.state != 'D') {
          w.job = w.jobs.join(" & ");
          this.pows.push(w);
        }
    ...
    return new Promise<any>(async (resolve) => {
      ...
      this.api._get("extraprofils/" + params["id"] + "/", "").subscribe((p: any) => {
        ...
        if (p) {
          ...
          p.links.push({
            url: "https://en.wikipedia.org/w/index.php?search=" + p.firstname + "+" + p.lastname,
            text: "Wikipedia"
          });
          p.links.push({
            url: "https://www.allocine.fr/rechercher/?q=" + p.firstname + "+" + p.lastname,
            text: "Allocine"
          });
  ...
  check_format(social: any) {
    if (this.profil[social.name] && this.profil[social.name].length > 0) {
      ...
      for(let format of social.formats){
        if (this.profil[social.name].startsWith(format))goodFormat=true
      }
      if (!goodFormat){
        social.message = "Format non reconnu"
      }else{
        social.message = "";
      }
    }else{
  ...
  refresh_awards() {
    this.awards = [];
    let l_awards = this.profil.award.sort((a, b) => {
  ...
  add_award() {
    this.api._get("festivals", "").subscribe((festivals: any) => {
      this.dialog.open(EditAwardComponent, {
        width: '350px', data: {
          profil: this.profil,
          pows: this.pows,
  ...
  async error_notify(work: any) {
    try {
      let warning = await _prompt(this, "Avez-vous vérifier la source de l'information ?", "",
        "Data Culture repose sur les contenus de IMDB / Unifrance. Avant de signaler une erreur il est préférable de vérifier l'exactitude de ses contenus",
        "oui/non",  "Signalez l'erreur","Voir la source")

      let message = await _prompt(this,
        "Signalez un problème", "",
        "Décrivez brievement la nature du problème (erreur sur le métier, oeuvre inconnue ...",
        "text", "Valider", "Annuler", false
      )
      if (message.length > 0) {
        work.error_notification = message
        this.api._patch("works/" + work.works[0], "", {error_notification:message}).subscribe(
          (r: any) => {
            showMessage(this, "Erreur signalée à l'administration")
            this.refresh_works()
          },
          )
      }
      ...
      for(let w of this.raw_works){
        if(w.id==work.works[0]){
          open(w.pow.links[0].url,"sources")
        }
      }
    }
  }
